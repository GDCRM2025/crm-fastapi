(() => {
  ...
  function validateStep(step, popup) {
    ...
    if (step === 1) {
      ...
      if (!/^OK:/i.test(allocation) && /error|inválid|inval|no hay productos|sin productos/i.test(`${calc} ${allocation}`)) {
  ...
  function upgrade(popup) {
    ...
    confirm.textContent = "Guardar y agendar";
    ...
    const showStep = (targetStep, options = {}) => {
      ...
      try { html.scrollTo({ top: 0, behavior: "auto" }); } catch (_) {}
    };
