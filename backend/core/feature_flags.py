...
FEATURES: dict[str, dict[str, Any]] = {
    ...
    "finance_sii": {
        "label": "GD Finance • SII",
        "description": "Conexiones SII, documentos recibidos, proveedores tributarios y cuentas por pagar.",
        "default_enabled": True,
        "menu_ids": ["finance_sii"],
    },
}
...
def feature_for_path(path: str) -> str | None:
    ...
    if p.startswith("/sgjo") or p.startswith("/rrhh/sgjo") or p.startswith("/rrhh/marcaciones"):
        return "attendance"
    if p.startswith("/api/finance/sii"):
        return "finance_sii"
    return None
