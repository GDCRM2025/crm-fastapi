...
def summary(days: int = Query(30, ge=1, le=730), user: dict = Depends(get_current_user)):
    ...
    with engine.connect() as conn:
        ...
        row = conn.execute(text("""
          SELECT COUNT(*) metric_rows,SUM(m.spend) spend,SUM(m.impressions) impressions,
                 SUM(m.clicks) clicks,SUM(m.platform_conversions) platform_conversions,
                 SUM(m.platform_conversion_value) platform_conversion_value,
                 MAX(m.ingested_at) last_data_at
    ...
    platform_data_available = bool(raw.pop("metric_rows", 0))
    return {"ok": True, "period": {"from": start, "to": date.today()}, "raw": raw,
            ...
            "data_availability": {
                "platform": "OBSERVED" if platform_data_available else "NO_DATA",
                "crm": "OBSERVED",
                "last_platform_data_at": raw.get("last_data_at"),
            },
