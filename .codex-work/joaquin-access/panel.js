...
function ensureFrameViewport(doc) {
  try {
    if (!doc || !doc.head || !doc.documentElement) return;
    let style = doc.getElementById("gd-viewport-guard");
    if (!style) {
      style = doc.createElement("style");
      style.id = "gd-viewport-guard";
    ...
    `;
    doc.documentElement.dataset.gdViewport = "responsive-scroll";
  } catch (_) {
  }
}
function initThemeToggle() {
  ...
  qs("#mainFrame").addEventListener("load", () => {
    ...
    try {
      const frame = qs("#mainFrame");
      ensureFrameViewport(frame == null ? void 0 : frame.contentDocument);
    } catch (_) {
    }
  ...
  try {
    ...
    if (r.ok) {
      ...
      if (perms) {
        ...
        if (ids.length && CURRENT_ROLE_ID !== 15) USER_MENU_ACCESS = new Set(ids);
      }
    }
  } catch (_) {
    USER_MENU_ACCESS = null;
...
function isFinanceSimpleMenuAllowed(item) {
  try {
    if (!item) return false;

    const me =
      (window.GD && window.GD.me)
      || {};
...
const MENU = [
  ...
  },
  {
    ...
    items: [
      ...
      { id: "gd_sales", label: "GD Sales Command Center V14.2 ↗", url: "/gd-sales/?v=20260824-control1", external: true }
    ]
  },
  {
    id: "gd_intelligence",
    ...
    items: [
      ...
      { id: "gdi_search_terms", label: "Search Terms", url: "/web/views/paid_media.html?v=20260810-2#terms", gdPermission: "paid_media_view" },
      { id: "gdi_seo", label: "Oportunidades SEO", url: "/web/views/paid_media.html?v=20260810-2#seo", gdPermission: "web_intelligence_seo" },
      { id: "gdi_alerts", label: "Alertas Intelligence", url: "/web/views/paid_media.html?v=20260810-2#alerts", gdPermission: "web_intelligence_view" },
      { id: "gdi_executive", label: "Dashboard Ejecutivo", url: "/web/views/paid_media.html?v=20260810-2#executive", gdPermission: "web_intelligence_view" },
  ...
  },
  {
    ...
    title: "Facturación",
    items: [
      { id: "billing_hub", label: "Facturación", url: "/web/views/billing_operational.html?v=20260817-op1", billingOperationalOnly: true }
    ]
  },
  {
    id: "alertas",
  ...
  },
  {
    ...
    title: "Finanzas",
    items: [
      { id: "finance_summary", label: "Resumen", url: "/web/views/finanzas_sii.html?v=20260824-r544#summary" },
      { id: "finance_payables", label: "Facturas / CxP", url: "/web/views/finanzas_sii.html?v=20260824-r544#invoices" },
      { id: "finance_reconciliation", label: "Conciliación bancaria", url: "/web/views/finanzas_sii.html?v=20260824-r544#reconciliation" },
      { id: "gast", label: "Clasificar cartolas", url: "/web/views/finanzas_simple.html?v=20260823-r4#cartolas" },
      { id: "finance_suppliers", label: "Proveedores", url: "/web/views/finanzas_sii.html?v=20260824-r544#suppliers" },
      { id: "pl", label: "P&L", url: "/web/views/finanzas_simple.html?v=20260823-r4#pnl" },
      { id: "finance_sii", label: "SII", url: "/web/views/finanzas_sii.html?v=20260824-r544#sii" }
    ]
  },
  {
    id: "operadores",
  ...
  },
  {
    ...
    items: [
  ...
  },
	  {
	    ...
	    title: "Tools",
	    items: [
	      { id: "tool_inbox", label: "Inbox omnicanal", url: "/web/views/omnichannel_inbox.html?v=20260810-1" },
	      { id: "tool_gmail", label: "Correo", url: "/web/views/tools.html?v=20260807-greenie-live-v1#correo" },
	      { id: "tool_ig", label: "Instagram", url: "/web/views/tools.html?v=20260807-greenie-live-v1#instagram" },
	      { id: "tool_wapp", label: "WhatsApp", url: "/web/views/tools.html?v=20260807-greenie-live-v1#whatsapp" },
	      { id: "tool_calc", label: "Calculadora", url: "/web/views/tools.html?v=20260807-greenie-live-v1#calc" },
	  ...
	  },
  {
    ...
    title: "Settings",
    items: [
      { id: "set_billing_tax", label: "Facturación tributaria", url: "/web/views/billing.html?v=20260817-tax1", superadminOnly: true },
...
function isBillingOperationalMenuAllowed(item) {
  return Boolean(
    item &&
    item.id === "billing_hub" &&
    (
...
function isBillingOperationalRoleAllowed(item) {
  if (!item || !item.billingOperationalOnly) return true;
  return isBillingOperationalMenuAllowed(item);
}
...
function listVisibleMenuItems() {
  ...
  for (const g of MENU) {
    for (const it of g.items) {
      ...
      if (!isBillingOperationalRoleAllowed(it)) continue;
      if (allowed && !allowed.has(it.id)) continue;
      if (USER_MENU_ACCESS && !USER_MENU_ACCESS.has(String(it.id)) && !isGdMenuAllowed(it) && !isBillingOperationalMenuAllowed(it) && !(it.superadminOnly && isCurrentSuperAdmin())) continue;
...
const PERMISSIONS = {
    ...
    "tools_hub",
	  ...
	  // CONTROL DE GESTION (id_rol=15): reportería y finanzas en modo consulta.
	  15: /* @__PURE__ */ new Set([
	    "dash_home",
	    "historial",
	    "rep_funnel",
...
try {
  ...
  // Control de Gestión tiene el mismo alcance operativo visible que Admin.
  // El borrado de leads sigue protegido por el backend y no depende del menú.
  if (PERMISSIONS[1]) {
    PERMISSIONS[15] = new Set(PERMISSIONS[1]);
  }
} catch (_) {
}
function buildMenu() {
  ...
  for (const g of MENU) {
    const visibleItems = allowed
      ? g.items.filter((it) => !it.sep && !it.noSidebar && !it.disabled && isItemFeatureEnabled(it.id) && allowed.has(it.id) && (!it.superadminOnly || isCurrentSuperAdmin()) && isBillingOperationalRoleAllowed(it) && (!USER_MENU_ACCESS || USER_MENU_ACCESS.has(String(it.id)) || isGdMenuAllowed(it) || isBillingOperationalMenuAllowed(it) || isFinanceSimpleMenuAllowed(it) || (it.superadminOnly && isCurrentSuperAdmin())) && (!it.driverOnly || isDriver))
      : g.items.filter((it) => !it.sep && !it.noSidebar && !it.disabled && isItemFeatureEnabled(it.id) && (!it.superadminOnly || isCurrentSuperAdmin()) && isBillingOperationalRoleAllowed(it) && (!USER_MENU_ACCESS || USER_MENU_ACCESS.has(String(it.id)) || isGdMenuAllowed(it) || isBillingOperationalMenuAllowed(it) || isFinanceSimpleMenuAllowed(it) || (it.superadminOnly && isCurrentSuperAdmin())) && (!it.driverOnly || isDriver));
    ...
    try {
      ...
      if (grew || grewOpen) {
        toast(`Correo: ${openTotal} pendiente(s)`, {
          kind: "ok",
          ms: 8e3,
	          onClick: () => openItem({ id: "tool_gmail", url: "/web/views/tools.html?v=20260807-greenie-live-v1#correo" })
	        });
	      }
