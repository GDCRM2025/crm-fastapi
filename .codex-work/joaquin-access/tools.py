...
from backend.core.agenda_lock import AgendaLockLease
...
def approve_agenda(
    ...
    lock_lease = AgendaLockLease(db.get_bind(), id_lead, wait_seconds=25)
    try:
        _ensure_lead_calendar_cols(db)
        ...
        try:
            got_lock = lock_lease.acquire()
        except Exception:
            got_lock = False
        if not got_lock:
            raise HTTPException(status_code=503, detail="Este lead todavía se está confirmando en otra sesión.")
