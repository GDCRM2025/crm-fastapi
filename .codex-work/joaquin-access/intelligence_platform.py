...
from backend.gd_intelligence.intelligence import actionable_alerts, executive_readiness, safe_ai_context, seo_opportunity_score, source_quality
from backend.gd_intelligence.openai_service import OpenAIServiceError, ask_openai
from backend.gd_intelligence.google_bi import GoogleBIConnection
...
class AIQuestionRequest(AIContextRequest):
    question: str = Field(min_length=3, max_length=500)


def _ai_services(conn) -> dict[str, Any]:
    return {
...
def data_quality(user: dict = Depends(get_current_user)):
    """Freshness and availability for every BI source, without secret metadata."""
    with engine.connect() as conn:
        _require(conn, user, "web_intelligence_view")
        rows = list(conn.execute(text("""
          SELECT 'CRM' source,true connected,COUNT(*)::bigint row_count,NULL::timestamptz last_sync,
            MAX(created_at) last_data_at,NULL::text error FROM leads
          UNION ALL SELECT 'TRACKING',(
            EXISTS(SELECT 1 FROM wi_sessions) OR EXISTS(SELECT 1 FROM wi_events)
            OR EXISTS(SELECT 1 FROM wi_integrations WHERE provider='TRACKING' AND enabled AND status IN ('CONNECTED','NO_DATA'))),
            COUNT(*)::bigint,(SELECT MAX(last_sync_at) FROM wi_integrations WHERE provider='TRACKING'),MAX(started_at),
            (SELECT MAX(last_error_safe) FROM wi_integrations WHERE provider='TRACKING' AND status='ERROR') FROM wi_sessions
          UNION ALL SELECT 'GA4',EXISTS(SELECT 1 FROM wi_integrations WHERE provider='GA4' AND enabled AND status IN ('CONNECTED','NO_DATA')),
            COUNT(*)::bigint,(SELECT MAX(last_sync_at) FROM wi_integrations WHERE provider='GA4'),MAX(ingested_at),
            (SELECT MAX(last_error_safe) FROM wi_integrations WHERE provider='GA4' AND status='ERROR') FROM wi_daily_metrics
          UNION ALL SELECT 'SEARCH_CONSOLE',EXISTS(SELECT 1 FROM wi_integrations WHERE provider='SEARCH_CONSOLE' AND enabled AND status IN ('CONNECTED','NO_DATA')),
            COUNT(*)::bigint,(SELECT MAX(last_sync_at) FROM wi_integrations WHERE provider='SEARCH_CONSOLE'),MAX(ingested_at),
            (SELECT MAX(last_error_safe) FROM wi_integrations WHERE provider='SEARCH_CONSOLE' AND status='ERROR') FROM wi_seo_query_daily
          UNION ALL SELECT 'PAGESPEED_LAB',EXISTS(SELECT 1 FROM wi_integrations WHERE provider='PAGESPEED' AND enabled AND status IN ('CONNECTED','NO_DATA')),
            COUNT(*)::bigint,(SELECT MAX(last_sync_at) FROM wi_integrations WHERE provider='PAGESPEED'),MAX(measured_at),
            (SELECT MAX(last_error_safe) FROM wi_integrations WHERE provider='PAGESPEED' AND status='ERROR') FROM wi_performance_runs WHERE source='LAB'
          UNION ALL SELECT 'CRUX_FIELD',EXISTS(SELECT 1 FROM wi_integrations WHERE provider='CRUX' AND enabled AND status IN ('CONNECTED','NO_DATA')),
            COUNT(*)::bigint,(SELECT MAX(last_sync_at) FROM wi_integrations WHERE provider='CRUX'),MAX(measured_at),
            (SELECT MAX(last_error_safe) FROM wi_integrations WHERE provider='CRUX' AND status='ERROR') FROM wi_performance_runs WHERE source='FIELD'
          UNION ALL SELECT 'SITE_HEALTH',true,COUNT(*)::bigint,MAX(finished_at),MAX(started_at),NULL::text FROM wi_site_health_runs
          UNION ALL SELECT 'GOOGLE_ADS',EXISTS(SELECT 1 FROM wi_ad_accounts WHERE platform='GOOGLE_ADS' AND enabled),
            COUNT(*)::bigint,(SELECT MAX(last_sync_at) FROM wi_ad_accounts WHERE platform='GOOGLE_ADS'),MAX(m.ingested_at),
            (SELECT MAX(last_error_safe) FROM wi_ad_accounts WHERE platform='GOOGLE_ADS' AND status='ERROR')
            FROM wi_ad_metrics_daily m JOIN wi_ad_accounts a ON a.id=m.account_id AND a.platform='GOOGLE_ADS'
          UNION ALL SELECT 'META_ADS',EXISTS(SELECT 1 FROM wi_ad_accounts WHERE platform='META_ADS' AND enabled),
            COUNT(*)::bigint,(SELECT MAX(last_sync_at) FROM wi_ad_accounts WHERE platform='META_ADS'),MAX(m.ingested_at),
            (SELECT MAX(last_error_safe) FROM wi_ad_accounts WHERE platform='META_ADS' AND status='ERROR')
            FROM wi_ad_metrics_daily m JOIN wi_ad_accounts a ON a.id=m.account_id AND a.platform='META_ADS'
        """)).mappings().all())
        google_ready = bool(conn.execute(text("SELECT to_regclass('public.wi_google_bi_connections') IS NOT NULL")).scalar())
        google_status = GoogleBIConnection().public_status(conn) if google_ready else {
            "provider_configured": False, "authorized": False, "verified": False,
        }
        ai_ready = bool(conn.execute(text("SELECT to_regclass('public.wi_ai_runs') IS NOT NULL")).scalar())
        if ai_ready:
            ai_latest = conn.execute(text("""
              SELECT status,error_code,started_at,finished_at
              FROM wi_ai_runs ORDER BY id DESC LIMIT 1
            """)).mappings().one_or_none()
            ai_successes = int(conn.execute(text("SELECT COUNT(*) FROM wi_ai_runs WHERE status='SUCCESS'")).scalar() or 0)
            error_messages = {
                "OPENAI_NOT_CONFIGURED": "Falta configurar OpenAI en el secret store del servidor.",
                "OPENAI_INVALID_CREDENTIAL": "OpenAI rechazó la credencial configurada.",
                "OPENAI_RATE_OR_QUOTA": "OpenAI no tiene cuota disponible o alcanzó temporalmente su límite.",
                "OPENAI_MODEL_CONFIGURATION": "El modelo de GD AI no está disponible para esta cuenta.",
                "OPENAI_UNAVAILABLE": "OpenAI no respondió en este momento.",
            }
            latest_error = str(ai_latest.get("error_code") or "") if ai_latest and ai_latest.get("status") == "ERROR" else ""
            rows.append({
                "source": "OPENAI",
                "connected": bool(ai_latest and ai_latest.get("status") == "SUCCESS"),
                "row_count": ai_successes,
                "last_sync": ai_latest.get("started_at") if ai_latest else None,
                "last_data_at": ai_latest.get("finished_at") if ai_latest and ai_latest.get("status") == "SUCCESS" else None,
                "error": error_messages.get(latest_error, "OpenAI devolvió un error seguro; revisa la configuración.") if latest_error else None,
            })
    policies = {"CRM": 24, "TRACKING": 2, "GA4": 36, "SEARCH_CONSOLE": 72, "PAGESPEED_LAB": 168,
                "CRUX_FIELD": 168, "SITE_HEALTH": 48, "GOOGLE_ADS": 36, "META_ADS": 36, "OPENAI": 24}
    items = []
    for row in rows:
        source = str(row["source"])
        google_source = source in {"GA4", "SEARCH_CONSOLE"}
        items.append(source_quality(
            source, connected=bool(row["connected"]), row_count=int(row["row_count"] or 0),
            last_sync=row["last_sync"], last_data_at=row["last_data_at"], error=row["error"],
            stale_after_hours=policies[source],
            provider_configured=google_status["provider_configured"] if google_source else None,
            authorized=google_status["authorized"] if google_source else None,
            verified=google_status["verified"] if google_source else None,
        ))
    return {"ok": True, "items": items, "null_policy": "0=valor observado; null=dato no disponible"}
...
def seo_opportunities(days: int = Query(90, ge=7, le=730), user: dict = Depends(get_current_user)):
    ...
    with engine.begin() as conn:
        ...
        output.sort(key=lambda item: item["score"] if item["score"] is not None else -1, reverse=True)
        for item in output:
            # Technical-only diagnostics belong to the technical SEO dimension,
            # not to the ranked opportunity table (whose schema requires a score).
...
def refresh_alerts(user: dict = Depends(get_current_user)):
    with engine.begin() as conn:
        ...
        health = conn.execute(text("""
          SELECT DISTINCT ON(h.site_id) h.id,h.site_id,s.code site_code,h.status,h.http_status,
            h.error_safe,h.missing_alt_count
          FROM wi_site_health_runs h JOIN wi_sites s ON s.id=h.site_id
...
def ai_context(payload: AIContextRequest, user: dict = Depends(get_current_user)):
    with engine.connect() as conn:
        ...
        services = _ai_services(conn)
    return {"ok": True, **safe_ai_context(str(user.get("role") or user.get("rol") or ""), payload.sources, services)}


@router.post("/ai/ask")
def ai_ask(payload: AIQuestionRequest, user: dict = Depends(get_current_user)):
    selected_model = str(os.getenv("GD_AI_MODEL") or "gpt-5.6-luna").strip()
    with engine.begin() as conn:
        _require(conn, user, "ai_use")
        context = safe_ai_context(
            str(user.get("role") or user.get("rol") or ""), payload.sources, _ai_services(conn)
        )
        run_id = conn.execute(text("""
          INSERT INTO wi_ai_runs(status,model,started_by,source_count)
          VALUES('RUNNING',:model,:actor,:sources) RETURNING id
        """), {"model": selected_model, "actor": _actor(user), "sources": len(context["sources"])}).scalar_one()
    try:
        result = ask_openai(payload.question, context, model=selected_model)
    except OpenAIServiceError as exc:
        with engine.begin() as conn:
            conn.execute(text("""
              UPDATE wi_ai_runs SET status='ERROR',error_code=:code,finished_at=now() WHERE id=:id
            """), {"id": run_id, "code": exc.code})
        action = "CONFIGURAR OPENAI" if exc.code == "OPENAI_NOT_CONFIGURED" else "REEMPLAZAR CREDENCIAL" if exc.code == "OPENAI_INVALID_CREDENTIAL" else "REVISAR CONFIGURACIÓN"
        raise HTTPException(503, {"code": exc.code, "message": str(exc), "action": action}) from exc
    with engine.begin() as conn:
        conn.execute(text("""
          UPDATE wi_ai_runs SET status='SUCCESS',response_characters=:characters,finished_at=now() WHERE id=:id
        """), {"id": run_id, "characters": len(result["answer"])})
    return {
        "ok": True,
